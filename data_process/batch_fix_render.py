"""
Batch re-render incomplete objects.
Reads from fix_render_queue.json and renders each object.
"""
import json
import os
import shutil
...
def render_object(args):
    ...
    # Check if already fully rendered
...
def main():
    queue_file = '/4T/CXY/MV-Painter/data/train_data/fix_render_queue.json'
    with open(queue_file) as f:
        render_queue = json.load(f)

    items = list(render_queue.items())
    print(f'Objects to re-render: {len(items)}')
