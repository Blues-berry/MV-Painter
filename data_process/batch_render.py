"""
Batch render objects using Blender.
Reads from render_queue.json and renders each object.
"""
import json
import os
import subprocess
...
def main():
    # Load render queue
    queue_file = '/4T/CXY/MV-Painter/data/train_data/render_queue.json'
    with open(queue_file) as f:
        render_queue = json.load(f)

    # Convert to list of tuples
    items = list(render_queue.items())
    print(f'Total objects to render: {len(items)}')

    # Process with multiprocessing (limit to 4 concurrent Blender instances)
