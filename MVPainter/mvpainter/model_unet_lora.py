...
from .lora_utils import save_lora_weights, create_lora_processors
...
class LoRACheckpointCallback(pl.Callback):
    """Saves only LoRA weights instead of full model checkpoints."""
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if (trainer.global_step + 1) % self.every_n_steps == 0 and trainer.global_rank == 0:
            # Use trainer.logdir if save_dir not specified
...
class MVDiffusionLoRA(pl.LightningModule):
    def __init__(
        ...
        self.img_size = 256
        self._lr = None  # Set by configure_optimizers via PL's base_learning_rate
        ...
        # Set up LoRA processors on the bare UNet
        lora_processors = create_lora_processors(
        ...
        if isinstance(pipeline.unet, UNet2DConditionModel) or True:
            # Already set processors, so don't replace them
        ...
        print(f"LoRA parameters: {lora_params / 1e6:.2f}M / {total_params / 1e6:.2f}M total ({100 * lora_params / total_params:.2f}%)")
    ...
    def configure_optimizers(self):
        ...
        print(f"LoRA trainable parameters: {sum(p.numel() for p in trainable_params) / 1e6:.2f}M")
