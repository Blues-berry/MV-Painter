...
print(f"Baseline entries: {sorted(baseline_entries)[:5]}")
print(f"Ours entries: {sorted(ours_entries)[:5]}")
...
plt.suptitle('Albedo: Baseline vs Ours', fontsize=14)
plt.tight_layout()
plt.savefig(f'{output_dir}/paper_main_comparison.png', dpi=150, bbox_inches='tight')
print('Saved paper_main_comparison.png')
plt.close()
...
for name, lf, c in [('Baseline','logs/baseline_73_final.log','steelblue'),('Ours','logs/ours_73_v2.log','coral')]:
    ...
    axes[0].plot(np.convolve(losses, np.ones(500)/500, 'valid'), color=c, linewidth=1.5, label=name)
axes[0].set_title('Loss Curve'); axes[0].set_xlabel('Step'); axes[0].set_ylabel('Loss')
axes[0].legend(); axes[0].grid(True, alpha=0.3); axes[0].set_ylim(0, 0.3)
...
axes[1].set_title('Loss Distribution'); axes[1].set_xlabel('Loss'); axes[1].legend(); axes[1].grid(True, alpha=0.3)

bars = axes[2].bar(['Baseline','Ours'], [407,53], color=['steelblue','coral'], width=0.5)
axes[2].set_title('Loss Spikes (>1.0)'); axes[2].set_ylabel('Count'); axes[2].grid(True, alpha=0.3, axis='y')
for b in bars:
    h = b.get_height()
    axes[2].annotate(f'{int(h)}', xy=(b.get_x()+b.get_width()/2,h), xytext=(0,3), textcoords='offset points', ha='center', fontsize=11)
plt.suptitle('Training Stability', fontsize=14); plt.tight_layout()
...
plt.suptitle('Ablation: Consistency Weight', fontsize=14); plt.tight_layout()
