...
Figure~\ref{fig:motivation} illustrates the training instability problem. The baseline loss curve exhibits frequent large spikes that exceed 1.0 (the red dashed line), while our method produces a much smoother curve with fewer and smaller spikes. These spikes can corrupt model checkpoints and waste hours of GPU time.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/motivation_loss_spikes.png}
\caption{Training loss comparison showing the instability problem. Top: Baseline exhibits frequent loss spikes exceeding 1.0. Bottom: Our MV-Consistency method produces a much smoother training curve.}
...
Multi-view image generation has seen rapid progress with diffusion-based architectures. Zero123++~\cite{zero123pp} generates six orthographic views simultaneously by extending Stable Diffusion with multi-view attention. SyncDreamer~\cite{syncdreamer} introduces synchronized multi-view denoising for 3D consistency, using elevation-aware conditioning to maintain coherence across views. Wonder3D~\cite{wonder3d} combines multi-view diffusion with cross-domain attention, generating both color and normal maps in a unified framework. InstantMesh~\cite{instantmesh} focuses on efficient mesh reconstruction from sparse views using FlexiCubes. MVPainter~\cite{mvpainter} provides a full pipeline with geometric control via ControlNet, enabling fine-grained control over multi-view generation.

While these methods achieve impressive visual quality, none explicitly address training stability. Our work fills this gap by providing a systematic analysis of training instability in multi-view generation and a practical solution through consistency regularization. The key difference is that existing methods rely on implicit architectural consistency (e.g., shared attention mechanisms), while we propose an explicit loss function that directly enforces cross-view consistency.

\subsection{PBR Material Generation}

IDArb~\cite{idarb} generates PBR materials from multi-view inputs using a domain-recursive UNet architecture that decomposes the generation process into albedo, normal, and metallic/roughness domains. Material Anything~\cite{materialanything} generates materials for any 3D object using a unified diffusion framework. Paint3D~\cite{paint3d} focuses on texture painting with UV-space optimization. These methods achieve high-quality results but do not report training stability metrics.

Our experiments reveal that training instability is a significant issue (407 spikes in 10,000 steps) that affects reproducibility and deployment readiness. This is particularly problematic for PBR generation because material properties must be consistent across views for realistic 3D rendering. A single loss spike can corrupt the model's understanding of material consistency, leading to artifacts in the final output.

\subsection{Training Regularization}

Common regularization techniques include EMA (Exponential Moving Average), gradient clipping, and label smoothing. EMA maintains a shadow copy of model weights that converges more smoothly, typically reducing spikes by approximately 10\%. Gradient clipping prevents gradient explosion by capping gradient norms, reducing spikes by approximately 15\%. Label smoothing softens target distributions, reducing overfitting but with modest stability improvement (approximately 5\%).

These methods improve generalization but do not specifically target multi-view consistency. Our MV-Consistency loss is orthogonal to these techniques and can be combined with them. As shown in Table~\ref{tab:regularization}, MV-Consistency provides 5-17$\times$ better stability than generic regularization methods with comparable quality impact.

\subsection{Cross-View Consistency in 3D}

Cross-view consistency has been explored in NeRF-based reconstruction, where multi-view consistency losses ensure that rendered views from different angles produce similar results. However, these methods operate in the rendering space rather than the training loss space. Our approach is fundamentally different: we add consistency regularization directly to the diffusion training objective, which provides gradient-level stability rather than output-level consistency.

\subsection{Training Stability in Diffusion Models}

Training stability in diffusion models has been addressed through various techniques including Min-SNR weighting, v-prediction, and noise schedule optimization. However, these methods are general-purpose and do not address the specific instability caused by multi-view gradient conflicts. Our MV-Consistency loss is the first method specifically designed to stabilize multi-view diffusion training by resolving gradient conflicts between views.
...
Figure~\ref{fig:method} provides an overview of our MV-Consistency method. The model processes multiple views of the same object through a shared UNet, producing predictions for albedo, normal, and material properties. We add an MSE loss between the predictions of different views, which provides a stabilizing gradient signal during training.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/method_overview.png}
\caption{MV-Consistency method overview. Input views are processed through a shared UNet to produce PBR predictions. We add an MSE loss between predictions of different views (red path), which stabilizes training by enforcing cross-view consistency.}
\label{fig:method}
\end{figure}
