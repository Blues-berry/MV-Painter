...
CANDIDATE_OBJECTS = [298,69,55,71,28,100,267,108,210,86,16,31,125,9,180,252,245,221,88,84,198,42,104,223,35,101,275,76,61,188,58,240,257,94,17,33,89,102,80,20,52,92,103,95,49,14,23,70,168,197,149,238,279,107,81,30,261,234,24,21,247,51,217,121,29,282,22,145,230,255,4,93,53,74,202,65,46,184,3,293]
...
def main():
    ...
    for obj_idx in CANDIDATE_OBJECTS:
        ...
        s250 = os.path.join(OUTPUT_DIR, f'obj_{obj_idx:03d}_s2.50.png')
        if os.path.exists(s125) and os.path.exists(s250):
            continue
        todo.append(obj_idx)
    
    print(f"Need to generate: {len(todo)} objects", flush=True)
    ...
    for i, obj_idx in enumerate(todo):
        ...
        for scale in scales:
            ...
            save_image(img, os.path.join(OUTPUT_DIR, f'obj_{obj_idx:03d}_s{scale:.2f}.png'))
            del img
