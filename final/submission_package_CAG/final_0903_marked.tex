...
%DIF ADD final_0903.tex   Sun Sep  6 07:35:30 2026
...
\DIFaddbegin \DIFadd{A preliminary version of this study was submitted to CAD/Graphics 2026 (Paper 75) \cite{cadgraphics2026p75}. This article extends that submission substantially: (i) the TCAS schedule is now grounded in an explicit two-stage epsilon-optimal selection rule (Proposition 1) with assumptions and proofs; (ii) the empirical protocol now separates a strictly disjoint 276-object holdout from the 24-object probe set, and C3 is evaluated unchanged on this holdout, with pooled 300-object statistics reported alongside; and (iii) the presentation, including the abstract, introduction, and conclusions, is substantially rewritten to state these extensions.
}
...
\DIFaddbegin \paragraph{\DIFadd{Perceptual validation with CLIP-IQA}}
...
\begin{table}[t]
\centering
\caption{\DIFaddFL{CLIP-IQA perceptual validation on 50 objects from the evaluation pool. Scores are reported for foreground regions and full rendered views; $\pm$ denotes the standard deviation across the 50 objects. Higher is better.}}
\label{tab:clipiqa}
...
}\DIFdelend \DIFaddbegin \DIFadd{For transparency, we note that an earlier version of this extension, evaluated on the main adapter under our initial protocol before the strictly paired and strictly disjoint training controls were adopted, had suggested a positive FAC gain. That preliminary comparison differed in both evaluation protocol and adapter instance, and it does not meet the controls we now consider necessary for a transfer claim; under the strictly paired, strictly disjoint protocol above, no trained configuration replicates the gain. We therefore report only the controlled re-examination and treat the earlier comparison as superseded.
}\DIFaddend 
...
Despite this variability, in every regime where scaling had a non-negligible effect, the temporal low-high-low schedule C3 yielded the best shape-texture balance. This indicates that TCAS's value is structural rather than incidental: by relying on the diffusion process's stage-dependent roles (global layout $\to$ geometry refinement $\to$ texture synthesis), it mitigates the harm of high-scale intervention regardless of the adapter's specific failure mode. The failure mode (flatten vs.\ artifact vs.\ no-op), its strength, and its temporal location are all adapter-dependent; \DIFdelbegin \DIFdel{but }\DIFdelend \DIFaddbegin \DIFadd{what remains stable in our experiments is that across the tested regimes }\DIFaddend the middle denoising stage \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{consistently emerged as }\DIFaddend the optimal window for geometric correction\DIFdelbegin \DIFdel{appears to be a property of the diffusion process itself}\DIFdelend \DIFaddbegin \DIFadd{, although whether this holds across adapter families remains an open question for future work.
}
...
\DIFdelbegin %DIFDELCMD < \bibitem{shao2025mvpainter}
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdel{Huang Z, et al. MV-Adapter: Multi-view consistent image generation made easy. arXiv:2412.03632, 2024.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibitem{chen2023text2tex}
...
\DIFadd{Huang Z, et al. MV-Adapter: Multi-view consistent image generation made easy. arXiv:2412.03632, 2024.
}

\bibitem{mou2023t2iadapter}
\DIFadd{Mou C, Wang X, Xie L, et al. T2I-Adapter: Learning adapters to dig out more controllable ability for text-to-image diffusion models. arXiv:2302.08453, 2023.
}
...
\DIFdelbegin %DIFDELCMD < \bibitem{wang2004ssim}
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdel{Oquab M, Darcet T, Moutakanni }\DIFdelend \DIFaddbegin \bibitem{kynkanniemi2024limited}
\DIFadd{Kynk\"a\"anniemi T, Aittala M, Karras }\DIFaddend T, et al. \DIFdelbegin \DIFdel{DINOv2: Learning robust visual features without supervision. }\DIFdelend \DIFaddbegin \DIFadd{Applying guidance in a limited interval improves sample and distribution quality in diffusion models. }\DIFaddend \textit{\DIFdelbegin \DIFdel{Transactions on Machine Learning Research}\DIFdelend \DIFaddbegin \DIFadd{Advances in Neural Information Processing Systems}\DIFaddend }, 2024.
...
\bibitem{cadgraphics2026p75}
\DIFadd{Anonymous. Adapter scaling trade-off and timestep-conditioned scheduling in multi-view diffusion texture generation. Submission \#75 of CAD/Graphics 2026, 2026.
}\DIFaddend 
