...
%DIF ADD final_0903.tex   Sun Sep  6 02:08:59 2026
...
\DIFadd{The low--high--low form is not assumed but derived from a measurable Correction--Alignment--Interference (CAI) stage-utility model; we state the selection rule formally. Let $Q(s_e,s_m,s_l)$ denote a fidelity objective (PSNR here), let $L$ denote the fixed-low schedule, let $R$ denote the texture-deviation risk (the artifact-sensitive texture diagnostics of Section~\ref{subsec:metrics}), and let $z_k\in\{0,1\}$ indicate whether the high scale is assigned to stage $k\in\{e,m,l\}$.
}

\begin{proposition}[Guardrailed $\epsilon$-optimal selection]
\label{prop:cai}
\DIFadd{Suppose (i) the stage-utility model $Q(s_e,s_m,s_l)=Q(L)+\sum_{k}U_k z_k$, where $U_k=Q(\text{high scale only in }k)-Q(L)$ is the measured finite-difference utility of stage $k$, and (ii) per-stage guardrails under which stage $k$ is admissible for the high scale only if its intervention does not reduce FG-SSIM by more than $\tau$ nor raise the texture-deviation risk beyond a factor $\rho$ relative to $L$. Selection proceeds in two stages. First, maximize fidelity over the admissible schedules $\mathcal{F}$: the fidelity-optimal assignment is $z_k^{\mathrm{fid}}=\mathbf{1}[\,U_k>0 \text{ and } k \text{ admissible}\,]$, with value $Q_{\max}=\max_{z\in\mathcal{F}}Q(z)$. Second, restrict attention to the $\epsilon$-equivalent set $\mathcal{F}_\epsilon=\{\,z\in\mathcal{F}: Q(z)\ge Q_{\max}-\epsilon\,\}$ for a small fidelity-equivalence margin $\epsilon>0$, and select $z^{\star}=\arg\min_{z\in\mathcal{F}_\epsilon}R(z)$. Then: an inadmissible stage is fixed at the conservative scale; any admissible stage whose utility alone exceeds $\epsilon$ must retain the high scale; and a stage whose utility is positive but whose utility's 95\% CI lower bound lies below $\epsilon$, with a structurally insignificant change, is fidelity-equivalent under either assignment, so the risk-dominant conservative scale is selected.
}\end{proposition}

\begin{quote}\small
\DIFadd{\textit{Proof.} Under (i), for any $z\in\mathcal{F}$, $Q(z)=Q_{\max}-\sum_{k:\,z_k^{\mathrm{fid}}=1}U_k\,(1-z_k)$, so $z\in\mathcal{F}_\epsilon$ exactly when the total positive utility dropped by reassigning high-scale stages to the conservative scale does not exceed $\epsilon$. Condition (ii) fixes $z_k=0$ for inadmissible stages and leaves the admissible stages independent, so each stage can be examined separately. A stage with $U_k>\epsilon$ cannot be dropped from the fidelity optimum without leaving $\mathcal{F}_\epsilon$, hence $z_k^{\star}=1$. A stage with $U_k>0$ whose utility's CI lower bound lies below $\epsilon$ contributes at most $\epsilon$, so both assignments lie in $\mathcal{F}_\epsilon$; within $\mathcal{F}_\epsilon$ the second selection stage minimizes texture-deviation risk, and for such a stage the structural change is insignificant while the conservative scale strictly reduces the risk, so $z_k^{\star}=0$. }\hfill\DIFadd{$\square$
}\end{quote}

\begin{remark}
\DIFadd{The utilities are measured finite differences on a 24-object stage ablation of the strong-residual, per-layer-capped adapter instance (regime~(b) of Section~\ref{subsec:adapterdep}), where per-stage effects are most visible: $U_e=-0.788$~dB (95\% CI $[-1.420,-0.156]$) with a guardrail violation (FG-SSIM $-0.105$, CI $[-0.141,-0.069]$; foreground Laplacian variance $3.85\times$ fixed-low), $U_m=+0.574$~dB $[0.391,0.756]$, and $U_l=+0.077$~dB $[0.006,0.149]$ with a structurally insignificant FG-SSIM change ($+0.001$, CI $[-0.002,0.003]$). Proposition~\ref{prop:cai} selects exactly the conservative--high--conservative assignment C3: the early stage is inadmissible, the middle stage must retain the high scale because its utility alone ($+0.574$~dB) exceeds any meaningful equivalence margin such as $\epsilon=0.1$~dB, and the late stage is fidelity-equivalent under either assignment ($+0.077$~dB, CI lower bound $+0.006$~dB, far below $\epsilon$), so the risk-dominant conservative scale is selected. The same conservative--high--conservative assignment is independently selected by the main adapter's 17-variant probe (Table~\ref{tab:probe}); the utilities above explain, rather than determine, that selection, and Remark~2 delimits the conditional scope of this explanation.
}\end{remark}
