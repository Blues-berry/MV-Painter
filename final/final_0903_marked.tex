...
%DIF ADD final_0903.tex   Fri Sep  4 07:45:42 2026
...
To address these issues, methods such as \DIFaddbegin \DIFadd{Paint3D, MVPaint, Hunyuan3D 2.0, and }\DIFaddend MVPainter formulate 3D texture generation as a geometry-controlled multi-view diffusion task\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{shao2025mvpainter}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, generating multi-view images under geometric control and baking them into UV textures \mbox{%DIFAUXCMD
\cite{zeng2024paint3d,cheng2024mvpaint,lai2025hunyuan3d,shao2025mvpainter}}\hspace{0pt}%DIFAUXCMD
...
\cite{zhang2023controlnet}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{lightweight parallel branches with a controllable scaling factor \mbox{%DIFAUXCMD
\cite{mou2023t2iadapter,zhang2023controlnet}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . IP-Adapter introduces image prompts through decoupled attention \cite{ye2023ipadapter}. LoRA and its variants implement lightweight model adaptation through low-rank residual updates \cite{hu2022lora,liu2024dora,zhang2023adalora}. Although these methods differ in architecture, they generally include some form of scaling factor that controls the influence of the external condition or adapter residual on the base model.
...
\DIFadd{The low--high--low form is not assumed but derived from a measurable Correction--Alignment--Interference (CAI) stage-utility model; we state the argument formally. Let $Q(s_e,s_m,s_l)$ denote a fidelity objective (PSNR here), let $L$ denote the fixed-low schedule, and let $z_k\in\{0,1\}$ indicate whether the high scale is assigned to stage $k\in\{e,m,l\}$.
}

\begin{proposition}[Guardrailed bang--bang optimality]
\label{prop:cai}
\DIFadd{Suppose (i) the stage-utility model $Q(s_e,s_m,s_l)=Q(L)+\sum_{k}U_k z_k$, where $U_k=Q(\text{high scale only in stage }k)-Q(L)$ is the measured finite-difference utility of stage $k$, and (ii) per-stage guardrails under which stage $k$ is admissible for the high scale only if its intervention does not reduce FG-SSIM by more than $\tau$ nor raise the artifact-sensitive Laplacian-variance diagnostic beyond a factor $\rho$ relative to $L$. Then the admissible schedule maximizing $Q$ is bang--bang: $z_k^{\star}=\mathbf{1}[\,U_k>0 \text{ and stage } k \text{ admissible}\,]$. Moreover, for a stage whose utility is positive but not separated from zero by a practically meaningful margin (a 95\% CI lower bound below a small equivalence margin $\epsilon$) and whose structural effect is insignificant, assigning the conservative scale yields a statistically indistinguishable fidelity at a strictly lower texture-deviation risk, so the conservative assignment is the risk-dominant choice.
}\end{proposition}

\begin{quote}\small
\textit{\DIFadd{Proof.}} \DIFadd{Under (i), $Q$ decomposes into the constant $Q(L)$ plus one independent binary term $U_k z_k$ per stage. Condition (ii) restricts each feasible set to $\{0,1\}$ (fixing $z_k=0$ for inadmissible stages) without coupling the stages; the joint maximizer over a product of independent binary feasible sets is therefore the product of per-stage maximizers, i.e., $z_k^{\star}=1$ exactly when $k$ is admissible and $U_k>0$. For the tie-break, if the CI of $U_k$ contains $0$, or its lower bound lies below the equivalence margin $\epsilon$ so that the gain is practically indistinguishable from $0$, and the structural change is insignificant, both assignments give statistically indistinguishable $Q$, while $z_k=0$ strictly reduces the texture-deviation risk. }\hfill\DIFadd{$\square$
}\end{quote}

\begin{remark}
\DIFadd{The utilities are measured finite differences on a 24-object stage ablation of the strong-residual, per-layer-capped adapter instance (regime~(b) of Section~\ref{subsec:adapterdep}), where per-stage effects are most visible: $U_e=-0.788$~dB (95\% CI $[-1.420,-0.156]$) with a guardrail violation (FG-SSIM $-0.105$, CI $[-0.141,-0.069]$; foreground Laplacian variance $3.85\times$ fixed-low), $U_m=+0.574$~dB $[0.391,0.756]$, and $U_l=+0.077$~dB $[0.006,0.149]$ with a structurally insignificant FG-SSIM change ($+0.001$, CI $[-0.002,0.003]$). Proposition~\ref{prop:cai} selects exactly the conservative--high--conservative assignment C3: the early stage is inadmissible, the middle stage is admissible with the dominant gain, and the late stage takes the conservative tie-break, since its utility gain is practically negligible (CI lower bound $+0.006$~dB, far below any meaningful equivalence margin such as $\epsilon=0.1$~dB). The same conservative--high--conservative assignment is independently selected by the main adapter's 17-variant probe (Table~\ref{tab:probe}); the utilities above explain, rather than determine, that selection, and Remark~2 delimits the conditional scope of this explanation.
}\end{remark}
...
\begin{table}[t]
\centering
\caption{Large-scale validation on \DIFdelbeginFL \DIFdelFL{300 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the full 300-object evaluation pool (pooled statistics; the pool contains the 24 probe }\DIFaddendFL objects\DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL . Values are absolute metrics of generated outputs with respect to ground truth.}
\label{tab:300obj}
\DIFdelbeginFL %DIFDELCMD < \resizebox{\columnwidth}{!}{%
%DIFDELCMD < \begin{tabular}{lcccc}
%DIFDELCMD < \toprule
...
\DIFadd{To address the concern that proxy texture statistics do not directly measure perceptual quality, we additionally evaluate CLIP-IQA \mbox{%DIFAUXCMD
\cite{wang2023clipiqa}}\hspace{0pt}%DIFAUXCMD
, which builds on CLIP image--text representations \mbox{%DIFAUXCMD
\cite{radford2021clip}}\hspace{0pt}%DIFAUXCMD
, on a 50-object subset of the evaluation pool as a complementary no-reference perceptual signal. CLIP-IQA is used as complementary perceptual evidence rather than a complete replacement for human preference evaluation.
}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{thebibliography}{40}
\DIFaddend 
...
\DIFadd{Zeng X, Chen X, Qi Z, et al. Paint3D: Paint anything 3D with lighting-immersive texture generation. }\textit{\DIFadd{Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition}}\DIFadd{, 2024.
}

\bibitem{cheng2024mvpaint}
\DIFadd{Cheng W, Mu J, Zeng X, et al. MVPaint: Synchronized multi-view diffusion for painting anything 3D. }\textit{\DIFadd{Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition}}\DIFadd{, 2025.
}
...
%DIFDELCMD < %%%
\DIFdel{Wang Z, Bovik A C, Sheikh H R, Simoncelli E P. Image quality assessment: From error visibility to structural similarity. }\textit{\DIFdel{IEEE Transactions on Image Processing}}%DIFAUXCMD
\DIFdel{, 2004, 13(4): 600--612.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{zhang2018lpips}
%DIFDELCMD < %%%
\DIFdel{Zhang R, Isola P, Efros A A, Shechtman E, Wang O. The unreasonable effectiveness of deep features as a perceptual metric. }\textit{\DIFdel{Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition}}%DIFAUXCMD
\DIFdel{, 2018.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{radford2021clip}
%DIFDELCMD < %%%
\DIFdel{Radford A, Kim J W, Hallacy C, et al. Learning transferable visual models from natural language supervision. }\textit{\DIFdel{International Conference on Machine Learning}}%DIFAUXCMD
\DIFdel{, 2021.
}%DIFDELCMD < 
...
\DIFadd{Wang Z, Bovik A C, Sheikh H R, Simoncelli E P. Image quality assessment: From error visibility to structural similarity. }\textit{\DIFadd{IEEE Transactions on Image Processing}}\DIFadd{, 2004, 13(4): 600--612.
}

\bibitem{zhang2018lpips}
\DIFadd{Zhang R, Isola P, Efros A A, Shechtman E, Wang O. The unreasonable effectiveness of deep features as a perceptual metric. }\textit{\DIFadd{Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition}}\DIFadd{, 2018.
}

\bibitem{wang2023clipiqa}
\DIFadd{Wang J, Chan K C K, Loy C C. Exploring CLIP for assessing the look and feel of images. }\textit{\DIFadd{Proceedings of the AAAI Conference on Artificial Intelligence}}\DIFadd{, 2023.
}

\bibitem{radford2021clip}
\DIFadd{Radford A, Kim J W, Hallacy C, et al. Learning transferable visual models from natural language supervision. }\textit{\DIFadd{International Conference on Machine Learning}}\DIFadd{, 2021.
}\DIFaddend 
