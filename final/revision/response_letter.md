...
4. **The FAC extension has been re-examined under a controlled protocol.** A strictly paired re-training and re-evaluation of the learned-correction (FAC) variants (same objects, seeds, and evaluation code; training pool strictly disjoint from evaluation objects) showed that unconstrained lightweight gating does not preserve the low--high--low envelope that makes TCAS effective; the revised manuscript reports this as a scoped negative result with the full training details requested by the reviewers, delimits the scenarios in which learned modulation can help, and keeps TCAS the sole, training-free method.
...
*(b) FAC.* The reviewers asked for the FAC training objective, optimization setup, and train/validation separation; we now provide all of them and report a controlled re-examination. In the revised manuscript (Sec. 4.6), the extension modules (LTAG/GSG/FSC, roughly 6×10³ trainable parameters) are trained with the same enhanced denoising objective as the base adapter (Adam, 2,000 steps) on the adapter's training pool, which is strictly disjoint from all evaluation objects, and are evaluated under a strictly paired protocol (identical objects, seeds, and evaluation code, with a warm-start control that recovers baseline results and thereby validates the evaluation path). In this controlled setting the learned variants did not improve upon the training-free baseline (full FAC −3.0 dB PSNR, paired win rate 1.3%, p ≈ 10⁻⁵⁰); the analysis shows that unconstrained gating flattens the low–high–low envelope that makes TCAS effective, and delimits the scenarios in which learned modulation can help (envelope-preserving constraints). TCAS, which requires no training, remains the main and sole method.
...
**Response.** Subset construction, selection, and overlap are now fully specified in Sec. 4.1 (see R1-C1): one 300-object Objaverse pool, disjoint from the 1,118-object training pool (verified zero identifier overlap); probe = first 24 identifiers; holdout = the remaining 276; the earlier probe–validation overlap is disclosed and all transfer claims use the disjoint holdout. For FAC: Sec. 4.6 now reports the full training configuration (roughly 6×10³ trainable parameters, same enhanced denoising objective, Adam, 2,000 steps, training pool strictly disjoint from evaluation objects) together with a controlled re-examination under a strictly paired protocol, which showed that unconstrained lightweight gating does not preserve the low–high–low envelope; the manuscript delimits when learned modulation can help and keeps TCAS as the main and sole method.
...
**Response.** Dataset and implementation details are addressed by the new Sec. 4.1 paragraph (R1-C1, R1-C2), including the verified zero-overlap between the training pool and the 300-object evaluation pool. Regarding FAC: Sec. 4.6 now specifies exactly what was asked — training data (the adapter's 1,118-object training pool, strictly disjoint from all evaluation objects; no evaluation object is used for training or model selection), loss function (the same enhanced denoising objective as the base adapter), optimization settings (Adam, 2,000 steps), and the additional parameter count (roughly 6×10³ trainable parameters in LTAG/GSG/FSC). Under this controlled paired protocol the learned variants did not improve upon the training-free baseline (full FAC −3.0 dB PSNR, win 1.3%, p ≈ 10⁻⁵⁰); the manuscript reports this as a scoped negative result, identifies the failure mechanism (unconstrained gating flattens the low–high–low envelope), and keeps the training-free TCAS as the main and sole method.
