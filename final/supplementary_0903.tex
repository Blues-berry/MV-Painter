...
\section{S1.\ \ Protocol}
All variants are trained and evaluated under the strictly paired protocol of Sec.~4.6 of the main manuscript: the base model, VAE, and GeoTex-Adapter remain frozen; only the LTAG/GSG/FSC controllers (roughly $6\times10^{3}$ parameters) are trained with the same enhanced denoising objective as the base adapter (Adam), on the adapter's training pool, which is strictly disjoint from all evaluation objects; evaluation uses 300 objects, the Euler discrete scheduler with 50 steps, seed 42, and identical evaluation code for every variant. Deficits are paired per object against the training-free TCAS baseline ($C3=(1.25,2.50,1.25)$).

\section{S2.\ \ Dose--response over seven trained configurations}

\begin{table}[h]
\centering\small
\caption{FAC controlled re-examination: seven trained configurations, all evaluated under the strictly paired protocol. $\Delta$PSNR and $\Delta$FG-SSIM are paired mean differences relative to the training-free TCAS baseline; the win rate is the fraction of objects on which the variant improves foreground PSNR. The untrained warm-start control (12-object paired subset, gate fixed at the exact C3 envelope) is reported in the text: $\Delta$FG-PSNR $=-0.18$~dB, 95\% CI $[-0.42,+0.05]$, statistically indistinguishable from the baseline.}
\label{tab:fac_dose}
\begin{tabular}{llcccc}
\toprule
Configuration & Temporal gate & FG-PSNR & FG-SSIM & $\Delta$PSNR & Win rate \\
\midrule
