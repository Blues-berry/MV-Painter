...
The low--high--low form is not assumed but derived from measurable stage utilities. Let $Q$ be a fidelity objective (PSNR here) and let $L$ denote the fixed-low schedule. For stage $k\in\{e,m,l\}$, define the finite-difference utility of activating the high scale only in that stage,
\begin{equation}
U_k = Q(\text{high in stage }k)-Q(L),
\label{eq:cai-utility}
\end{equation}
together with an evidence vector of the corresponding FG-SSIM and texture-diagnostic changes, which acts as a guardrail against structural collapse or artifact inflation. Under the additive stage approximation $Q(s_e,s_m,s_l)\approx Q(L)+\sum_k U_k z_k$ with $z_k\in\{0,1\}$, the optimization is separable and admits a bang--bang solution: choose the high scale where $U_k>0$ and the conservative scale elsewhere. On a 24-object stage ablation, the measured utilities are $U_e=-0.788$~dB (95\% CI $[-1.420,-0.156]$), $U_m=+0.574$~dB $[0.391,0.756]$, and $U_l=+0.077$~dB $[0.006,0.149]$: early-high violates the structural guardrail (FG-SSIM $-0.105$, CI $[-0.141,-0.069]$), late-high is structurally neutral, and mid-high carries the useful fidelity gain, which selects the conservative--high--conservative assignment. The additive model is a conditional stage-selection approximation---stage effects interact, and the complete C3 schedule is therefore validated directly; changing the adapter or its scale semantics requires re-estimating $U_k$ (Section~\ref{subsec:adapterdep}).
...
It should be emphasized that FAC requires learning a small number of additional parameters. Therefore, TCAS, which requires no training, remains the main method in this paper. FAC is used only as a learned adaptive extension; in our controlled experiments (Section~\ref{subsec:facexp}) it did not improve upon the training-free schedule, so we report it as a negative-result check rather than a contribution.
...
TCAS uses a fixed three-stage scale, which has the advantages of requiring no training, being simple to implement, and introducing low computational overhead. However, its residual scaling is still uniform across spatial positions and frequency components, lacking fine-grained adaptivity. To examine whether stage-wise adapter control can be extended to learned adaptive correction, we trained the FAC extension---LTAG for timestep gating, GSG for spatial gating, and FSC for frequency-selective correction on top of the TCAS temporal schedule---and evaluated it on the 300-object validation set under a strictly paired protocol: the extension modules were trained only on the adapter's training pool, which is disjoint from all evaluation objects, and both training-free and learned variants were evaluated with identical objects, seeds, and evaluation code.

Under this controlled protocol, the learned variants did not improve upon the training-free baseline. Relative to the TCAS baseline, LTAG-only lost about 1.0~dB PSNR, LTAG+GSG lost about 1.0~dB PSNR with no FG-SSIM gain, and the full FAC variant lost about 3.0~dB PSNR with a paired per-object win rate of only 1.3\% ($p < 10^{-50}$). A warm-start control, in which the learned gate was initialized at the exact C3 schedule and recovered baseline-level results, confirms that the evaluation path was correct: the degradation is a real training effect. Inspection of the trained gates shows the underlying mechanism---the learned temporal gate flattens the low--high--low envelope that makes TCAS effective, raising the early- and late-stage intervention that the fixed schedule deliberately keeps conservative.

We therefore do not claim learned correction as a contribution of this paper. TCAS, which is training-free and plug-and-play, remains the main and sole method. This negative result is informative: the fixed middle-stage concentration is a strong default, and lightweight learned gating trained with the same objective does not automatically recover, let alone improve, the hand-designed schedule.
...
Finally, our controlled re-evaluation of the learned FAC extension showed that lightweight learned gating does not automatically improve upon the fixed schedule; we therefore report FAC as a negative result and keep TCAS, which is training-free, as the main and sole method.
...
As an additional check, a controlled learned-correction extension (FAC) did not improve upon the training-free schedule under a strictly paired protocol; we report this negative result to delimit the scope of the stage-wise control idea, whose core instantiation is the training-free TCAS.
