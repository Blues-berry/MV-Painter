...
In this controlled setting, the learned variants did not improve upon the training-free baseline. Relative to the TCAS baseline, LTAG-only lost about 1.0~dB PSNR, LTAG+GSG lost about 1.0~dB PSNR with no FG-SSIM gain, and the full FAC variant lost about 3.0~dB PSNR with a paired per-object win rate of only 1.3\% ($p<10^{-50}$); adding the spatial and frequency controllers on top of the drifting temporal gate rescued only 15.7\% of objects relative to LTAG-only. Two further controls localize the failure. A warm-start control, in which the learned gate is initialized at the exact C3 envelope and not trained, is statistically indistinguishable from TCAS (paired $\Delta=-0.18$~dB on the 12-object control, 95\% CI $[-0.42,+0.05]$, crossing zero), validating the evaluation path. Conversely, a two-stage control that freezes the gate at the exact C3 envelope and trains only the spatial and frequency controllers still degrades fidelity ($-2.56$~dB PSNR, paired wins 4/300), although less than unconstrained full FAC. Three additional configurations---an envelope penalty that constrains the gate to stay near C3 during joint training, a fidelity-weighted objective under the frozen gate, and a gentle schedule with fourfold-reduced training---confirm the pattern ($-2.74$, $-2.37$, and $-1.10$~dB, respectively): the degradation increases monotonically with the amount of controller training, mild training merely trades signal fidelity for a slightly higher structural statistic, and no configuration reaches the training-free schedule. Together these controls show that preserving the temporal envelope is necessary but not sufficient: the remaining damage is driven by the learned-modulation training signal itself, which trades foreground fidelity for denoising-objective loss, while unconstrained temporal drift removes the envelope protection entirely.

This outcome delimits the scenario in which learned modulation affects the paper's conclusions. Whenever the controllers are left untrained with the envelope preserved, the extension neither helps nor harms and all main conclusions remain unchanged; every trained variant we tested under this objective degraded fidelity, with or without envelope preservation. Learning a modulation that \emph{improves upon} the C3 schedule therefore requires more than architectural lightness---it needs a training signal that does not trade away foreground fidelity (e.g., fidelity-weighted objectives, envelope-preserving constraints, or post-hoc rather than gradient-learned gates). With the tested lightweight controllers, the fixed schedule is the stronger default, and TCAS, which is training-free and plug-and-play, remains the main and sole method of this paper.
