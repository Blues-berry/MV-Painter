...
%DIF ADD final_0903.tex   Thu Sep  3 07:57:06 2026
...
\DIFdelend \DIFaddbegin \author[a]{\DIFadd{Xueyuan Che}\corref{cor1}\DIFaddend }
\DIFaddbegin \cortext[cor1]{Corresponding author.}
\author[a]{\DIFadd{Libo Sun}}
...
\DIFadd{The low--high--low form is not assumed but derived from measurable stage utilities; we state the argument formally. Let $Q(s_e,s_m,s_l)$ denote a fidelity objective (PSNR here), let $L$ denote the fixed-low schedule, and let $z_k\in\{0,1\}$ indicate whether the high scale is assigned to stage $k\in\{e,m,l\}$.
}
...
\begin{quote}\small
\textit{\DIFadd{Proof.}} \DIFadd{Under (i), $Q$ decomposes into the constant $Q(L)$ plus one independent binary term $U_k z_k$ per stage. Condition (ii) restricts each feasible set to $\{0,1\}$ (removing at most the value $1$ for inadmissible stages) without coupling the stages; the joint maximizer over a product of independent binary feasible sets is therefore the product of per-stage maximizers, i.e., $z_k^{\star}=1$ exactly when $k$ is admissible and $U_k>0$. For the tie-break, if the CI of $U_k$ contains $0$ and the structural change is insignificant, both assignments give statistically indistinguishable $Q$, while $z_k=0$ strictly reduces the texture-deviation risk. }\hfill\DIFadd{$\square$
}\end{quote}
...
\DIFadd{To address the concern that proxy texture statistics do not directly measure perceptual quality, we additionally evaluate CLIP-IQA, which builds on CLIP representations, on a 50-object validation subset as a complementary no-reference perceptual signal. CLIP-IQA is used as complementary perceptual evidence rather than a complete replacement for human preference evaluation.
}
